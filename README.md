...
A local demo of the MOOC platform, structured as a **course catalog**: Python, Java, and C are
each a separate course with its own Easy/Medium/Hard practice levels, its own final exam, and its
own certificate. Register/login, pick a course, solve problems in an in-browser Monaco editor
...
# install the three runtimes this app uses (each takes a few minutes — Python alone is ~1GB)
...
Optional: add an `OPENAI_API_KEY` to `backend/.env` (copy from `.env.example`) to get real AI
...
the exam → certificate ready. Dashboard aggregates progress across all three courses.
...
backend/
  ...
  ai.py               AI hint endpoint (stub if no OPENAI_API_KEY)
  ...
  seed.py             3 courses (python/java/c) × 8 problems each + hardcoded admin account
...
Each course (Python/Java/C) has 8 problems split Easy/Medium/Hard. A level counts as complete
...
- Only 8 problems per course; exam retries can start repeating questions once you've used them
  ...
  widen the pool for all three courses at once.
